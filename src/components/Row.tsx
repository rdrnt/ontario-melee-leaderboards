  ...
  return (
    <tr className={`${playerRank.bgClass} border-separate border-spacing-2 border-b-2 border-gray-600`} >
      ...
      <td className="text-gray-100 md:px-6 md:py-4 p-1 whitespace-nowrap text-center overflow-hidden md:max-w-full max-w-[7rem] text-elipses">
        <a className="md:text-base text-base max-w-xs text-gray-300 hover:text-gray-500 hover:underline font-medium" href={codeToUrlSlug(player.connectCode.code)}>{player.displayName}</a>
        <div className="text-gray-300 text-sm">{player.connectCode.code}</div>
        <div className="text-gray-300 text-xs italic">{player.region.city ? `${player.region.city} (${player.region.province})` : `(${player.region.province})`}</div>
